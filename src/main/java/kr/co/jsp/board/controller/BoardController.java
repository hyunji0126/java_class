...
public class BoardController extends HttpServlet {
	...
	protected void doRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "regist":
			 ...
			 * board_list.jsp에는 데이터베이스로부터 전체 글 목록을 가져오는 로직이 없으니까요. (단순히 보여주는 역할만 할 뿐이다.)
...
//			ㄴ>sendRedirect를 사용하면 안되는 이유 :
//			request객체에 list를 담아서 전달하려 하는데, 
//			sendredirect를 사용하면 응답이 나가면서 request객체가 소멸해 버립니다.
...
//			-> RequestDispatcher
		...
		case "delete":
			...
			sv.execute(request, response);//값을 받아 일해라~
			response.sendRedirect("/MyWeb/list.board");
			break;
